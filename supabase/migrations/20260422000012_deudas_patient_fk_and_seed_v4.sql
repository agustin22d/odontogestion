-- =============================================================
-- Odonto Gestión — FK patient_id en deudas + view por_cobrar v2 + seed v4
--
-- Para que al elegir un paciente en el form de cobranzas podamos
-- traer sus deudas pendientes, necesitamos linkear `deudas.patient_id`.
-- Antes solo había match por nombre (best-effort).
-- =============================================================

ALTER TABLE deudas
  ADD COLUMN IF NOT EXISTS patient_id UUID REFERENCES pacientes(id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS idx_deudas_patient ON deudas(patient_id) WHERE patient_id IS NOT NULL;

-- Re-CREATE la vista para incluir patient_id (el front filtra por él)
...
-- =============================================================
-- Helper: aplicar un pago a una deuda (incrementa monto_cobrado +
-- recalcula estado). SECURITY DEFINER porque updaten deudas necesita
-- por_cobrar.manage que el caller debería tener; igual checkeamos
-- adentro contra clinic_id del usuario.
-- =============================================================
...
-- =============================================================
-- seed_demo_data v4: pobla patient_id en deudas (delta vs v3)
-- =============================================================
