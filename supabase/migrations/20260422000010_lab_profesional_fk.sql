-- =============================================================
-- Odonto Gestión — FK profesional_id en laboratorio_casos
--
-- La migración 20260422000007 agregó profesional_id a `turnos` pero
-- olvidó hacer lo mismo en `laboratorio_casos`. Esto lo agrega y
-- re-CREATE OR REPLACE la función seed_demo_data (idéntica a la
-- versión 20260422000009 — el código ya esperaba esta columna).
--
-- Después de aplicar, si querés repoblar el demo:
--   SELECT seed_demo_data('TU_CLINIC_ID');
-- =============================================================

ALTER TABLE laboratorio_casos
  ADD COLUMN IF NOT EXISTS profesional_id UUID REFERENCES profesionales(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_lab_profesional ON laboratorio_casos(profesional_id)
  WHERE profesional_id IS NOT NULL;

-- Re-CREATE OR REPLACE de la función seed (no cambia el cuerpo, solo
-- aseguramos que el seed referencie la columna recién creada).
